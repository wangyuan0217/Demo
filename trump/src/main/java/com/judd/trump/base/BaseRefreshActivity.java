...
import static com.judd.trump.app.Config.HIDE_LIST_END_TIP;
import static com.judd.trump.app.Config.PAGE_FROM;
import static com.judd.trump.app.Config.REFRESH_DELAY;

public abstract class BaseRefreshActivity<CELL, LIST_CELL> extends TActivity
    ...
    protected void bindView() {
        if (canRefresh) {
            try {
                ...
                showToast("no swipeRefreshLayout");
            }
        }
    }
    ...
    protected void updateAfterLoad(List<CELL> model) {
        ...
        if (currentPage == Config.PAGE_FROM)
        ...
        if (model.size() == 0) {
            mAdapter.loadMoreEnd(HIDE_LIST_END_TIP);
            return;
        }
    ...
    protected MyCallback myCallback = new MyCallback<LIST_CELL>() {
        ...
        public void onSuccess(LIST_CELL model, String message, int code) {
            BaseRefreshActivity.this.onSuccess(model, message, code);
        }
