...
public class TimeUtil {
    ...
    public static double getDays(String time1, String time2) {
        long end = getTimestampWithampm(time2);
        long start = getTimestampWithampm(time1);
        long between = end - start;
        if (between >= 0)
            return 0.5 * (between / 43200000) + 0.5;
        else
            return -1;
    }

    public static long getTimestampWithampm(String time) {
        long timestamp = 0;
        if (time.endsWith("上午")) {
            timestamp = string2Milliseconds(time.replace("上午", " 00:00:00"));
        } else if (time.endsWith("下午")) {
            timestamp = string2Milliseconds(time.replace("下午", " 12:00:00"));
        }
        return timestamp;
    }
